...
// Initialize SQLite database - USING THE NEW DATABASE FILE
...
app.post('/api/projects', (req, res) => {
  const { title, description, date, location, image, type, status, impact, beneficiaries } = req.body;
  db.run(
    'INSERT INTO projects (title, description, date, location, image, type, status, impact, beneficiaries) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [title, description, date, location, image, type, status, impact, beneficiaries],
    function(err) {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`✅ Using NEW database: ${dbPath}`);
